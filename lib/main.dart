...
class DashboardBootstrapScreen extends StatelessWidget {
  ...
  Future<void> _showCreateHabitDialog(BuildContext context) async {
    ...
    final habitName = await showDialog<String>(
      ...
      builder: (dialogContext) {
        return AlertDialog(
          title: const Text('Them thoi quen'),
          content: TextField(
            controller: controller,
            autofocus: true,
            textInputAction: TextInputAction.done,
            decoration: const InputDecoration(
              labelText: 'Ten thoi quen',
              hintText: 'Vi du: Uong nuoc',
            ),
            onSubmitted: (_) {
    ...
    if (exists) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Thoi quen nay da ton tai.')),
      );
      return;
    }
    ...
    if (provider.errorMessage == null) {
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(const SnackBar(content: Text('Da tao thoi quen moi.')));
    }
  }
  ...
  Future<void> _cleanupAutoGeneratedHabits(BuildContext context) async {
    ...
    if (count == 0) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Khong co habit tu dong de xoa.')),
      );
      return;
    }
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (dialogContext) {
        return AlertDialog(
          title: const Text('Don du lieu habit'),
          content: Text(
            'Phat hien $count habit auto-generated (dang habit_<timestamp>). Ban co muon xoa khong?',
          ),
          actions: [
            TextButton(
    ...
    if (removed > 0) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Da xoa $removed habit auto-generated.')),
      );
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            tooltip: 'Don habit auto-generated',
